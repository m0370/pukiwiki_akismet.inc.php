...
v2.1.0（2026.6.13）では以下の修正を行いました。導入方法・設定方法はこれまでと変わりません（プラグインファイルの差し替えだけでアップデートできます）。なおPHPのcurl拡張が必要です（通常のレンタルサーバーでは有効になっています）。

- キャプチャ認証通過後にトップページに飛ばされて入力内容が消えてしまうことがある問題を修正しました。投稿先の特定に失敗した場合も、入力内容を保持した再投稿フォームを表示します。
- キャプチャのトークンが未送信のとき認証通過扱いにしていた問題を修正しました（cmd=akismet への直接POSTでキャプチャを迂回できる脆弱性でした）。未送信時はキャプチャフォームを再表示します。
- スパムログ閲覧（?cmd=akismet）の logfile パラメータをホワイトリスト化し、サーバ上の任意ファイルが読み出せる問題と反射型XSSを修正しました。あわせてログ閲覧をデフォルトで管理者限定にしました（PLUGIN_AKISMET_SPAMLOG_ADMIN_ONLY）。
- Akismet APIとの通信をHTTPS（443番ポート）化し、reCAPTCHA検証通信のSSL証明書検証を有効化しました。
- Akismet APIの呼び出し形式を現行の公式ドキュメントに合わせました（接続先を rest.akismet.com に固定し、APIキーは api_key パラメータとしてPOST本文で送信。実サーバで動作確認済み）。
...
- スパムログ記録時の逆引きDNSをオプション化（デフォルト無効）しました。
...
- PLUGIN_AKISMET_AUTOPOST_AFTER_SUBMITHAM
  - スパム取り消し報告後に自動で本来の投稿内容を自動投稿。デフォルトで有効。不具合が出るようなら FALSE にしてください。本来の投稿内容を表示するだけで自動投稿はしなくなります。念のために用意しておきました。v2.1.0では自動投稿に失敗した場合も入力内容を保持した再投稿フォームを表示します。
- PLUGIN_AKISMET_LOG_REVERSE_DNS
  - スパムログ記録時にIPアドレスの逆引きDNSを行ってホスト名を記録する。デフォルトで無効（生のIPアドレスを記録）。スパムの大量POST時に逆引き待ちでサーバが詰まるのを防ぐため、v2.1.0からオプション化しました。
- PLUGIN_AKISMET_SPAMLOG_ADMIN_ONLY
  - スパムログ閲覧（?cmd=akismet）を管理者限定にする。デフォルトで有効（v2.1.0から）。ログには訪問者のIPアドレスやUser Agentが含まれるためです。従来どおり誰でも閲覧できるようにするには FALSE にしてください。管理者判定は「管理者ならばスパムチェックしない」と同じ仕組み（Basic認証）を使います。
...
アクション型でアクセス (index.php?cmd=akismet) するとスパムフィルタログがみれます。v2.1.0からはデフォルトで管理者のみ閲覧できます（PLUGIN_AKISMET_SPAMLOG_ADMIN_ONLY を参照）。

ちなみに、ログファイルは PukiWiki 本家では cache/spamlog.txt に Plus! では log/spamlog.txt に保存されています。ログには投稿者のIPアドレス等が含まれるため、ログファイルにWebから直接アクセスされないようサーバ設定（.htaccess等）で保護されていることを確認してください（PukiWiki標準構成では保護されています）。
